...
object ThemeUtil {
    ...
    fun updateTheme(activity: Activity) {
        ...
        val theme = sharedPreferences.getString("nexuratube_theme", "System")!!
        when (theme) {
            "Light" -> {
        ...
        val iconMode = sharedPreferences.getString("nexuratube_icon", "Default")!!
    ...
    fun getStyledAppName(context: Context): Spanned {
        ...
        return "<span  style='color:$hexColor';>Nexura</span>tube"
    ...
    fun updateAppIcon(activity: Activity, theme: String, appIconMode: String) {
        ...
        for (appIcon in availableIcons) {
            val activityClass = "com.nexuratube.svg." + appIcon.activityAlias
        ...
        when (iconMode) {
            "LightIcon" -> {
                activity.packageManager.setComponentEnabledSetting(
                    ComponentName(activity.packageName, "com.nexuratube.svg.LightIcon"),
            ...
            "DarkIcon" -> {
                activity.packageManager.setComponentEnabledSetting(
                    ComponentName(activity.packageName, "com.nexuratube.svg.DarkIcon"),
                    PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
                    PackageManager.DONT_KILL_APP
                )
            }
            "BlueIcon" -> {
                activity.packageManager.setComponentEnabledSetting(
                    ComponentName(activity.packageName, "com.nexuratube.svg.BlueIcon"),
                    PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
                    PackageManager.DONT_KILL_APP
                )
            }
            "GreenIcon" -> {
                activity.packageManager.setComponentEnabledSetting(
                    ComponentName(activity.packageName, "com.nexuratube.svg.GreenIcon"),
                    PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
                    PackageManager.DONT_KILL_APP
                )
            }
            // or "System"
                ...
                //set dynamic icon
                activity.packageManager.setComponentEnabledSetting(
                    ComponentName(activity.packageName, "com.nexuratube.svg.Default"),
                    PackageManager.COMPONENT_ENABLED_STATE_ENABLED,
                    PackageManager.DONT_KILL_APP
                )
            }
