package com.nexuratube.svg.database.repository

import android.content.Context
import android.util.Patterns
import androidx.preference.PreferenceManager
import com.nexuratube.svg.database.DBManager.SORTING
import com.nexuratube.svg.database.dao.CommandTemplateDao
import com.nexuratube.svg.database.dao.ResultDao
import com.nexuratube.svg.database.models.ChapterItem
...
import com.nexuratube.svg.database.repository.HistoryRepository.HistorySortType
import com.nexuratube.svg.database.viewmodel.HistoryViewModel
import com.nexuratube.svg.database.viewmodel.ResultViewModel
import com.nexuratube.svg.util.Extensions.getIDFromYoutubeURL
import com.nexuratube.svg.util.Extensions.isYoutubeChannelURL
import com.nexuratube.svg.util.Extensions.isYoutubeURL
import com.nexuratube.svg.util.Extensions.isYoutubeWatchVideosURL
import com.nexuratube.svg.util.Extensions.needsDataUpdating
import com.nexuratube.svg.util.FileUtil
import com.nexuratube.svg.util.extractors.GoogleApiUtil
import com.nexuratube.svg.util.extractors.YoutubeApiUtil
import com.nexuratube.svg.util.extractors.newpipe.NewPipeUtil
import com.nexuratube.svg.util.extractors.ytdlp.YTDLPUtil
...
class ResultRepository(private val resultDao: ResultDao, commandTemplateDao: CommandTemplateDao, private val context: Context) {
    val NEXURATUBE_SEARCH = "NEXURATUBE_SEARCH"
...
//        }
    ...
    private suspend fun getYoutubeVideo(inputQuery: String, resetResults: Boolean, addToResults: Boolean) : List<ResultItem>{
        ...
        if (resetResults) {
            ...
            res.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = NEXURATUBE_SEARCH }
        }
        if (addToResults){
            val ids = resultDao.insertMultiple(res)
            ids.forEachIndexed { index, id ->
    ...
    private suspend fun getFromYTDLP(inputQuery: String, resetResults: Boolean, addToResults: Boolean, singleItem: Boolean = false) : List<ResultItem> {
        ...
        ytdlpUtil.getFromYTDL(inputQuery, singleItem) { results ->
            ...
            results.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = NEXURATUBE_SEARCH }
            if (addToResults) {
    ...
    suspend fun updateDownloadItem(
        ...
        if (downloadItem.needsDataUpdating()){
            runCatching {
                ...
                if (downloadItem.playlistTitle.isEmpty() || (downloadItem.playlistTitle.isNotBlank() && downloadItem.playlistTitle != NEXURATUBE_SEARCH)   ) downloadItem.playlistTitle = info.playlistTitle
