...
import com.nexuratube.svg.database.viewmodel.CommandTemplateViewModel
...
import com.nexuratube.svg.receiver.ShareActivity
    ...
    override fun setupDialog(dialog: Dialog, style: Int) {
        ...
        if (!disableUpdateData) {
            if(result.title.isEmpty() && currentDownloadItem == null && !sharedPreferences.getBoolean("quick_download", false) && type != DownloadType.command){
                ...
                val usingGenericFormatsOrEmpty = result.formats.isEmpty() || result.formats.any { it.format_note.contains("nexuratubegeneric") }
                ...
                kotlin.runCatching {
                    lifecycleScope.launch(Dispatchers.Main) {
                        if (result.size == 1 && result[0] != null) {
                            ...
                            val usingGenericFormatsOrEmpty = res.formats.isEmpty() || res.formats.any { it.format_note.contains("nexuratubegeneric") }
